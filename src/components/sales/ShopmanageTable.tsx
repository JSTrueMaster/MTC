...
export default class Shopmanage extends React.Component<any, any> {
      ...
      <>
        ...
        <div className="gird-container h-full" onKeyDown={this.onKeyDown} onKeyUp={this.onKeyUp}>
          ...
          <div className="flex flex-col relative igr-table h-full">
            ...
            {/* Bottom Button */}
            <div className={`flex ${!this.state.addRowHidden && 'hidden'} absolute overflow-x-auto overflow-y-hidden w-full bottom-[43px]`} onScroll={this.handleScroll}>
              <input className="addRow-input !w-[16.6%]" placeholder="店舗名" value={this.state.addRowData['店舗名']} onChange={(event) => this.onChangedAddRow('店舗名', event.target.value)} />
              <div className="flex addRow-input !w-[16.6%] min-w-[150px] 店舗種別">
                <Select className="w-6/12 my-auto h-[34px] !w-[100%]" value={(this.state.addRowData['店舗種別']) ? this.state.addRowData['店舗種別'] : ""} sx={{
                  "fieldset": {
                    border: 'none',
                  }
                }}
                  onChange={(event) => this.onChangedAddRow('店舗種別', event.target.value)} >
                  {
              ...
              <input className="addRow-input !w-[16.6%]" placeholder="都道府県" value={this.state.addRowData['都道府県']} onChange={(event) => this.onChangedAddRow('都道府県', event.target.value)} />
              <input className="addRow-input !w-[16.6%]" placeholder="住所" value={this.state.addRowData['住所']} onChange={(event) => this.onChangedAddRow('住所', event.target.value)} />
              <input className="addRow-input !w-[16.6%]" placeholder="代理ログイン" value={this.state.addRowData['代理ログイン']} onChange={(event) => this.onChangedAddRow('代理ログイン', event.target.value)} />
        ...
        >
          <div className="flex flex-col w-full h-full border-none">
            <div className="flex w-full h-[70px] px-[40px] bg-[#BCD8F1] rounded-t-[20px]">
              <h1 className="font-Meiryo text-[20px] my-auto">経費一覧</h1>
            </div>
            <div className="bg-white p-10 overflow-y-auto rounded-b-[20px]">

              {/* 店舗名 & 支払口座 */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">店舗名</label>
              ...
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">支払口座</label>
                <Select className="w-8/12 my-auto h-[34px]" id="支払口座"
              ...
              {/* 郵便番号 */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">郵便番号</label>
                <TextField className="w-8/12 my-auto"
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      height: 34,
              ...
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">都道府県</label>
                <TextField className="w-8/12 my-auto"
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      height: 34,
              ...
              {/* 住所 */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">住所</label>
                <TextField className="w-8/12 my-auto"
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      height: 34,
              ...
              {/* 住所 */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">電話番号</label>
                <TextField className="w-8/12 my-auto"
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      height: 34,
              ...
              {/* メールアドレス */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-4/12 my-auto pl-10">メールアドレス</label>
                <TextField className="w-8/12 my-auto"
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      height: 34,
              ...
              <div className="flex justify-center mx-auto w-full my-5 pr-5">
                <button className="flex justify-center items-center w-[113px] h-[34px] rounded-[17px] px-[14px] py-[9px] text-[14px] text-white bg-gray" onClick={(e) => { e.preventDefault(); this.setState({ openDetailModal: false }) }} >キャンセル</button>
  ...
  public onSaveRow = async (e: IgrTemplateCellUpdatingEventArgs) => {
    ...
    if (e.cellInfo.rowItem['id']) {
      const res = await api.client.updateShop(e.cellInfo.rowItem['id'], e.cellInfo.rowItem)
  ...
  public onAddRow = () => {
    ...
    const new_data = { '店舗名': '', '店舗種別': '', '都道府県': '', '住所': '', '代理ログイン': '', '操作': 0, }
  ...
  public onOpenDetailModal = async (row: any) => {
    ...
    // const res = await api.client.getStoreHistory(id)
