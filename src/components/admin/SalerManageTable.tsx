...
export default class SalerManageTable extends React.Component<any, any> {
  ...
  public onGetData = async () => {
    ...
    const res = await api.client.getSaler(params)
        ...
        <div className="gird-container h-full" onKeyDown={this.onKeyDown} onKeyUp={this.onKeyUp}>
          ...
          <Box className="flex flex-row justify-end">
            ...
            <Button component="span" sx={{
              ...
              onClick={(event) => {
                ...
                if (this.permission[12] == 'true' || this.userRole == 'super_admin') {
                  event.preventDefault()
                  this.onExportExcel("卸先一覧")
                } else {
          ...
          <div className="flex flex-col relative igr-table h-full">
            ...
            <IgrDataGrid
              ...
              primaryKey={["id"]}
              editMode={(this.permission[10] == 'true' || this.userRole == 'super_admin') ? 1 : 0}
              summaryScope="none"
              ...
              width="100%"
            >
              <IgrComboBoxColumn field="卸先名" dataSource={this.state.storeName} />
              <IgrTextColumn field="姓名" />
              <IgrTextColumn field="電話番号" />
              <IgrTextColumn field="メールアドレス" />
              <IgrTextColumn field="郵便番号" />
              <IgrComboBoxColumn field="都道府県" dataSource={storetype} />
            ...
            <div className={`flex ${!this.state.addRowHidden && 'hidden'} absolute overflow-x-auto overflow-y-hidden w-full bottom-[50px]`} onScroll={this.handleScroll} onClick={this.handlClick}>
              <div className="flex addRow-input !w-1/6 min-w-[150px]">
                <Select className="w-6/12 my-auto h-[34px] !w-[100%]" value={(this.state.addRowData['卸先名']) ? this.state.addRowData['卸先名'] : ""} sx={{
                  "fieldset": {
                    border: 'none',
                  }
                }}
                  onChange={(event) => this.onChangedAddRow('卸先名', event.target.value)} >
                  {
                    this.state.storeName.map((item: any, key: any) => {
                      return (
              ...
              <input className="addRow-input !w-1/6 min-w-[150px]" placeholder="姓名" value={this.state.addRowData['姓名']} onChange={(event) => this.onChangedAddRow('姓名', event.target.value)} />
              <input className="addRow-input !w-1/6 min-w-[150px]" placeholder="電話番号" value={this.state.addRowData['電話番号']} onChange={(event) => this.onChangedAddRow('電話番号', event.target.value)} />
              <input className="addRow-input !w-1/6 min-w-[150px]" placeholder="メールアドレス" value={this.state.addRowData['メールアドレス']} onChange={(event) => this.onChangedAddRow('メールアドレス', event.target.value)} />
              <input className="addRow-input !w-1/6 min-w-[150px]" placeholder="郵便番号" value={this.state.addRowData['郵便番号']} onChange={(event) => this.onChangedAddRow('郵便番号', event.target.value)} />
              <div className="flex addRow-input !w-1/6 min-w-[150px]">
                <Select className="w-6/12 my-auto h-[34px] !w-[100%]" value={(this.state.addRowData['都道府県']) ? this.state.addRowData['都道府県'] : ""} sx={{
                  "fieldset": {
                    border: 'none',
                  }
                }}
                  onChange={(event) => this.onChangedAddRow('都道府県', event.target.value)} >
                  {
              ...
              <input className="addRow-input !w-1/6 min-w-[150px]" placeholder="住所" value={this.state.addRowData['住所']} onChange={(event) => this.onChangedAddRow('住所', event.target.value)} />
        ...
        >
          <div className="flex flex-col w-full h-full border-none">
            <div className="flex w-full h-[70px] px-[40px] bg-[#BCD8F1] rounded-t-[20px]">
              <h1 className="font-Meiryo text-[20px] my-auto">卸先詳細</h1>
            </div>
            <div className="bg-white p-10 overflow-y-auto rounded-b-[20px]">

              {/* 卸先名 & 支払口座 */}
              <div className="flex w-full my-5 pr-5">
                <label className="w-3/12 my-auto pl-10">卸先名</label>
                <Select className="w-9/12 my-auto h-[34px]" id="卸先名"
              ...
              <div className="flex w-full my-5 pr-5">
                ...
                <Select className="w-9/12 my-auto h-[34px]" id="都道府県"
                ...
                >
                  {
                    storetype.map((item, key) => {
                      return (
                        <MenuItem value={`${item.value}`} key={`郵便番号${key}`}>
                          {item.value}
                        </MenuItem>
                      )
                    })
  ...
  public onActionCellUpdating = (s: any, e: IgrTemplateCellUpdatingEventArgs) => {
    ...
    if (e.cellInfo.rowItem['操作'] == 1) {
      ...
      if (content.childElementCount === 0) {
        ...
        span1.onclick = event => {
          if (this.permission[10] == 'true' || this.userRole == 'super_admin') {
            event.preventDefault()
        ...
        span2.onclick = e => {
          if (this.permission[11] == 'true' || this.userRole == 'super_admin') {
            e.preventDefault()
  ...
  public onSaveRow = async (e: IgrTemplateCellUpdatingEventArgs) => {
    ...
    if (e.cellInfo.rowItem['id']) {
      const res = await api.client.updateSaler(e.cellInfo.rowItem['id'], e.cellInfo.rowItem)

      //add update history
      let ownHistoryData = [] as any
      let otherHistoryData = [] as any
      this.changedHistoryData.forEach((element: any) => {
        if (e.cellInfo.rowItem['卸先名'] == element['saler_name']) {
      ...
      const res_history = await api.client.updateSalerHistory(e.cellInfo.rowItem['id'], ownHistoryData)
      this.changedHistoryData = otherHistoryData

    } else {
      const res = await api.client.createSaler(e.cellInfo.rowItem)
    }
  ...
  public onDeleteRow = async () => {
    ...
    const res = await api.client.deleteSaler(row['id'])
  ...
  public onAddRow = () => {
    ...
    if (this.permission[9] == 'true' || this.userRole == 'super_admin') {
      const new_data = { '卸先名': '', '姓名': '', '電話番号': '', 'メールアドレス': '', '郵便番号': '', '都道府県': '', '住所': '', '操作': 0, }
  ...
  public onAddRowSave = async () => {
    try {
      const res = await api.client.createSaler(this.state.addRowData)
      if (res.status == 201) {
  ...
  public onOpenDetailModal = async (row: any) => {
    ...
    const res = await api.client.getSalerHistory(id)
  ...
  public onSaveDetailModal = async () => {
    ...
    if (this.state.modalRow['id']) {
      ...
      const res = await api.client.updateSaler(this.state.modalRow['id'], data)
    } else {
      const res = await api.client.createSaler(this.state.modalRow)
    }
