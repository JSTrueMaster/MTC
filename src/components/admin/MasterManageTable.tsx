...
export default class MasterManageTable extends React.Component<any, any> {
      ...
      <>
        ...
        >
          <div className="text-[14px] grid grid-cols-6 gap-2 place-content-evenly w-8/12 text-center">
            <div
              className={`${
                this.state.selectedSort == "商品カテゴリ" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
            ...
            <div
              className={`${
                this.state.selectedSort == "商品シリーズ" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
            ...
            <div
              className={`${
                this.state.selectedSort == "商品ランク" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
            ...
            <div
              className={`${
                this.state.selectedSort == "ブランド" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
            ...
            <div
              className={`${
                this.state.selectedSort == "来店動機A" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
            ...
            <div
              className={`${
                this.state.selectedSort == "来店動機B" && "text-[#1DAEFF]"
              } cursor-pointer hover:bg-[#1DAEFF]-700`}
              onClick={(event) => {
          ...
          <div className="flex flex-col relative igr-table h-full">
            ...
            <IgrDataGrid
              ...
              primaryKey={["id"]}
              editMode={
                this.permission[31] == "true" || this.userRole == "super_admin"
                  ? 1
                  : 0
              }
              summaryScope="none"
            ...
            >
              <input
                className="addRow-input !w-1/2 min-w-[150px]"
                placeholder={this.state.selectedSort}
                value={this.state.addRowData[this.state.selectedSort] || ""}
                onChange={(event) =>
                  this.onChangedAddRow(
  ...
  public onSaveRow = async (e: IgrTemplateCellUpdatingEventArgs) => {
    ...
    if (e.cellInfo.rowItem["id"]) {
      ...
      updateMasterData["item_name"] =
        e.cellInfo.rowItem[this.state.selectedSort];
      const res = await api.client.updateMaster(
      ...
      this.changedHistoryData.forEach((element: any) => {
        if (
          e.cellInfo.rowItem["category"] == element["field"] &&
          e.cellInfo.rowItem[this.state.selectedSort] == element["master_name"]
        ) {
      ...
      const res_history = await api.client.updateMasterHistory(
        e.cellInfo.rowItem["id"],
        ownHistoryData
      );
      this.changedHistoryData = otherHistoryData;
  ...
  public onAddRow = () => {
    if (this.permission[30] == "true" || this.userRole == "super_admin") {
      const new_data = {
        category: this.state.selectedSort,
        公開店舗: [""],
        操作: 0,
      };
  ...
  public onSaveDetailModal = async () => {
    ...
    if (this.state.modalRow["id"]) {
      ...
      modal_data["公開店舗"] =
        typeof modal_data["公開店舗"] == "string"
          ? modal_data["公開店舗"].split(",")
          : modal_data["公開店舗"];
      const res = await api.client.updateMaster(
        this.state.modalRow["id"],
        modal_data
      );
    } else {
