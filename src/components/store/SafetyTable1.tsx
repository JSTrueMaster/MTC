...
export default class SafetyTable1 extends React.Component<any, any> {
  ...
  public getSafety1 = async () => {

    const res = await api.store.getSafety1({
          ...
          <Box className="flex flex-row justify-end">
            ...
            <Button component="span" sx={{
              ...
              onClick={(event) => {
                ...
                this.onExportExcel("小口現金確認表")
              }}>
              Excelダウンロード
            </Button>
          </Box>

          <div className="igr-table h-full" onKeyDown={this.onKeyDown} onKeyUp={this.onKeyUp}>
            {/* Table */}
            <IgrDataGrid
              ref={this.onGridRef}
            ...
            >
              ...
              <IgrNumericColumn field="経費出費" isEditable={false} showGroupingSeparator="true" />
              <IgrNumericColumn field="小口入金" isEditable={false} showGroupingSeparator="true" />
              <IgrNumericColumn field="小口残高" isEditable={false} showGroupingSeparator="true" />
  ...
  public onSaveChangedData = async () => {
    ...
    if (changedData.length > 0) {
      const result = await api.store.createSafety1(changedData)
  ...
  //upload csv
  public onUploadXlsx1 = (event: any) => {
    ...
    reader.onload = async (e) => {
      ...
      jsonData.forEach((element: any, index) => {
        if (index > 1 && element[0]) {
          final_results.push({
            '日付': element[0] ? this.convertDate(element[0]) : '',
            '店舗名': storeName,
            '10,000円': element[1] ? element[1] : 0,
            '5,000円': element[2] ? element[2] : 0,
            '1,000円': element[3] ? element[3] : 0,
            '500円': element[4] ? element[4] : 0,
            '100円': element[5] ? element[5] : 0,
            '50円': element[6] ? element[6] : 0,
            '10円': element[7] ? element[7] : 0,
            '5円': element[8] ? element[8] : 0,
            '1円': element[9] ? element[9] : 0,
          });
        }
      });

      await api.store.createSafety1(final_results).then((response: any) => {
