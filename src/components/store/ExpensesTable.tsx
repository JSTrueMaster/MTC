        ...
        >
          ...
          <div className="flex flex-col relative igr-table h-full">
            ...
            >
              ...
              <IgrTemplateColumn field="操作" width={'*>110'} pinned='right' cellUpdating={this.onActionCellUpdating} />
        ...
        Papa.parse(csv, {
          complete: function (results) {
            ...
            const headers: string[] = [
              'No', '計上月', '支払日', '勘定科目', '支払口座\n/小口', '支払先', '内容', '金額', '経費対象', '備考', '書類',
            ];
            ...
            for (let i = 1; i < results.data.length; i++) {
              ...
              for (let j = 0; j < innerArray.length; j++) {
                ...
                if (headers.indexOf(header) !== -1) {
                  //TODO FOR DATATYPE
                  if (header == 'No') { row[header] = i }
                  else if (header == '計上月') { if (element == '') row[header] = null; else row[header] = element.replace('年', '-').replace('月', '-1'); }
                  else if (header == '支払日') { if (element == '' || element == '未払計上') row[header] = null; else row[header] = element }
                  else if (header == '支払口座\n/小口') { row['支払口座/小口'] = element; }
                  else if (header == '金額') { row[header] = element.replace('(', '-').replace(')', '').replace(/,/g, '').replace(/¥/g, '') }
