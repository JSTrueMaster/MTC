...
export default class CfsTable extends React.Component<any, any> {
  ...
  public onGetData = async () => {
    ...
    this.accounts.forEach((account: string) => {
      sumObj[account] = this.accountsVal[account]
    })
    ...
    temp.forEach((item: any) => {
      ...
      this.accounts.forEach((account: string) => {
        this.accountsVal[account] = item[account]
      })
        ...
        Papa.parse(csv, {
          complete: function (results) {
            ...
            const headers: string[] = [
              'No', '年月日', '口座名', '入金', '出金', '現預金残高', '科目', '相手先', '備考１'
            ];
            ...
            for (let i = 1; i < results.data.length; i++) {
              ...
              for (let j = 0; j < innerArray.length; j++) {
                ...
                if (headers.indexOf(header) !== -1) {
                  ...
                  else if (header == '現預金残高') { 現預金残高 += row['入金'] - row['出金'], row[header] = 現預金残高 }
